...
export default async function manageSystemSupport() {
  ...
  const reportProductList = pendingSupports.filter(
    (support) => support.support_type === "Report Product",
  );
  const reportSellerList = pendingSupports.filter(
    (support) => support.support_type === "Report Shop",
  );
  const reportGeneralList = pendingSupports.filter(
    (support) =>
      support.support_type !== "Report Product" &&
      support.support_type !== "Report Seller",
  );
    ...
    <div className={Styles.contentPage}>
      <div className={Styles.upperPart}>
        <div className={Styles.pageDescription}>
          <h1>Admin Report Process</h1>
          <p>
            Admin are able to manage the request that are rejected from seller
            or is not response from seller within certain period.
          </p>
        </div>
      ...
      <div className={Styles.refundsOverviewContainer}>
        <ReportOverViewBar
          ...
          reportProductCount={reportProductCount}
          reportSellerCount={reportSellerCount}
          reportGeneralCount={reportGeneralCount}
        />
      </div>
...
export async function ReportOverViewBar({
  allReportCount,
  reportProductCount,
  reportSellerCount,
  reportGeneralCount,
...
export function ReportDataAnalyticsComponent({ title, count }) {
  return (
    <div className={Styles.analyticsComponent}>
      <div className={Styles.analyticsTitle}>{title}</div>
      <div className={Styles.countContainer}>
        <span className={Styles.countWrapper}>{count}</span>
      </div>
    </div>
  );
}
